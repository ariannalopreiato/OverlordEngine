...
void SoftwareSkinningScene_3::Update()
{
	...
	XMMATRIX bone0Transform = XMMatrixMultiply(XMLoadFloat4x4(&m_pBone0->GetBindPose()), XMLoadFloat4x4(&m_pBone0->GetTransform()->GetWorld()));
	XMMATRIX bone1Transform = XMMatrixMultiply(XMLoadFloat4x4(&m_pBone1->GetBindPose()), XMLoadFloat4x4(&m_pBone1->GetTransform()->GetWorld()));
	...
	for (int i = 0; i < m_SkinnedVertices.size(); ++i)
	{
		...
		XMStoreFloat3(&bone0, XMVector3TransformCoord(vertex, bone0Transform));
		XMStoreFloat3(&bone1, XMVector3TransformCoord(vertex, bone1Transform));

		auto blend = XMLoadFloat3(&bone0) * m_SkinnedVertices[i].blendWeight0 + XMLoadFloat3(&bone1) * m_SkinnedVertices[i].blendWeight1;
		XMStoreFloat3(&m_SkinnedVertices[i].transformedVertex.Position, blend);
	}
